...
var listaElementosCAT = [
  ...
  new ElementoFormato("Causado por Intervención Antrópica","spinnerMM","intervencionAnt",["No Aplica","Si","No"]),
  new ElementoFormato("IMPORTANCIA","spinnerMM","IMPORTANC",["No Aplica","Alta","Media","Baja"]),
  new ElementoFormato("ENCUESTADOR","spinnerMM","ENCUESTAD",["No Aplica","DAVID ZAPATA VALENCIA","LUIS ALFREDO MARTÍNEZ FRANCO","ANGELY PAOLA ARCILA QUINTERO","SANTIAGO CORREA ESTRADA","LUISA FERNANDA CARDONA PRECIADO","LAURA SANCHEZ MESA","EZEQUIEL FERRO PALACIOS","ANA MARÍA VALENCIA LONDOÑO","YULIETH LOPERA ORTÍZ","CAROLINA GARCÍA CADAVID"]),
  new ElementoFormato("FECHA EVENTO (aaaa-mm-dd)","edittextMM","FECHA_MOV",0),
  new ElementoFormato("FUENTE FECHA EVENTO","spinner","FECHA_FUENTE",["No Aplica","Incierta al Momento","Sensores remotos","Reportada por terceros","Consultada en pagina web"]),
  new ElementoFormato("CONFIABILIDAD FECHA EVENTO","spinnerMM","ConfiFechaMM",["No Aplica","Exacta","Certeza mes y año","Certeza Año","Confiabilidad baja","Incierta"]),
...
var listaElementosINV = [
  ...
  new ElementoFormato("ENCUESTADOR","spinnerMM","ENCUESTAD",["No Aplica","DAVID ZAPATA VALENCIA","LUIS ALFREDO MARTÍNEZ FRANCO","ANGELY PAOLA ARCILA QUINTERO","SANTIAGO CORREA ESTRADA","LUISA FERNANDA CARDONA PRECIADO","LAURA SANCHEZ MESA","EZEQUIEL FERRO PALACIOS","ANA MARÍA VALENCIA LONDOÑO","YULIETH LOPERA ORTÍZ","CAROLINA GARCÍA CADAVID"]),
  new ElementoFormato("FECHA EVENTO (aaaa-mm-dd)","edittextMM","FECHA_MOV",0),
  new ElementoFormato("FUENTE FECHA EVENTO","spinnerMM","FECHA_FUENTE",["No Aplica","Incierta al Momento","Sensores remotos","Reportada por terceros","Consultada en pagina web"]),
  new ElementoFormato("CONFIABILIDAD FECHA EVENTO","spinnerMM","ConfiFechaMM",["No Aplica","Exacta","Certeza mes y año","Certeza Año","Confiabilidad baja","Incierta"]),
  new ElementoFormato("FECHA REPORTE (aaaa-mm-dd)","edittextMM","FECHA_REP",0),
  // new ElementoFormato("SIMMA","edittextMM","COD_SIMMA",0),
  new ElementoFormato("Municipio",  "spinnerMM",  "NOM_MUN", ['VALPARAISO']),
  ...
  new ElementoFormato("HUMEDAD",  "radiobtnMM",  "humedad", [ "No Aplica","Mojado","Muy Húmedo","Húmedo","Ligeramente húmedo","Seco"]),
  new ElementoFormato("PLASTICIDAD",  "radiobtnMM",  "plasticidad", ["No Aplica","Alta","Media","Baja","No plástico"]),
  new ElementoFormato("ORIGEN SUELO",  "radiocheck",  "origensuelo", ["Residual","Coluvial (Depósito de vertiente)","Volcánico","Sedimetario"]),
  new ElementoFormato("TIPO DEPÓSITO (Origen Sedimentario)",  "radiocheck",  "tipodeposito", ["Aluvial","Eolico","Glacial","Lacustre","Marino"]),
  new ElementoFormato("VELOCIDAD",  "spinner",  "velocidad", [ "No Aplica","Extr. rápido (>5 m/s)","Muy rápido (>3 m/min)","Rápido (>1.8 m/hr)","Moderado (>13 m/mes)","Lento (>1.6 m/año)","Muy lento (>16 mm/año)","Extr. Lento (Menor a 16 mm/año)"]),
...
function GraficarEstacion(isEdit, id, addInEdit){ 
  ...
  if (isEdit) {
    if(estaciones["estacion_"+id]["Formularios"].count_VIVIENDA>0){
      for (let j = 0; j < estaciones["estacion_"+id]["Formularios"].count_VIVIENDA; j++) {
        var formato = estaciones["estacion_"+id]["Formularios"]["Form_VIVIENDA"]["Form_VIVIENDA_"+j]; 
        if (formato.activo) {
    ...
    if (addFormato == 'VIVIENDA') {
      if (addInEdit) {
        var auxContVIVIENDA = estaciones["estacion_"+id]["Formularios"].count_VIVIENDA;
        if (estaciones["estacion_"+id]["Formularios"]["Form_VIVIENDA"] === undefined) {
          estaciones["estacion_"+id]["Formularios"]["Form_VIVIENDA"] = {};
        }
        estaciones["estacion_"+id]["Formularios"]["Form_VIVIENDA"]["Form_VIVIENDA_"+auxContVIVIENDA] ={activo:true};
        GraficarVIVIENDA(isEdit, auxContVIVIENDA, id, true);
        estaciones["estacion_"+id]["Formularios"].count_VIVIENDA = auxContVIVIENDA+1;
        contAddVIVIENDA++;
      }
      else{
  ...
  function GraficarVIVIENDA(isEdit, j, id, addInEdit) {
    ...
    if (isEdit || addInEdit) {
      if (isEdit) {
        formato = estaciones["estacion_"+id]["Formularios"]["Form_VIVIENDA"]["Form_VIVIENDA_"+j]; 
...
function GuardarEstacion(isEdit, id) {
  ...
  if (isEdit) {
    ...
    formVIVIENDAcount = estaciones['estacion_'+id]['Formularios']['count_VIVIENDA'];
    ...
    formVIVIENDAcount = contAddVIVIENDA;
  ...
  formFormatos['count_VIVIENDA'] = formVIVIENDAcount;
  ...
  if (isEdit) {
    for (let i = 0; i < formVIVIENDAcount; i++) {
      VIVIENDAActivos = estaciones['estacion_'+id]['Formularios']['Form_VIVIENDA']['Form_VIVIENDA_'+i]['activo'];
      formatosVIVIENDA.push({'activo': VIVIENDAActivos});
    }
    ...
    for (let i = 0; i < formVIVIENDAcount; i++) {
      formatosVIVIENDA.push({'activo':true});
    }
    for (let i = 0; i < formUGSRcount; i++) {
  ...
  for (const attr in idsFormatos) {
    ...
    // console.log(auxAttr);
    if(auxAttr[0]=='VIVIENDA'){
      formatosVIVIENDA[auxAttr[2]][auxAttr[1]] = $('#'+idsFormatos[attr]).val();
    }
    if(auxAttr[0]=='UGSR'){
  ...
  if (isEdit) {
    for (let i = 0; i < formVIVIENDAcount; i++) {
      if (estaciones['estacion_'+id]['Formularios']['Form_VIVIENDA']['Form_VIVIENDA_'+i]['activo']) {
        formFormatos['Form_VIVIENDA']['Form_VIVIENDA_'+i]=formatosVIVIENDA[i];
      }else{
        formFormatos['Form_VIVIENDA']['Form_VIVIENDA_'+i]={
          "activo": false,
          "ObsValpa": "",
          "idformatoValpa": "",
          "invValpa": "",
